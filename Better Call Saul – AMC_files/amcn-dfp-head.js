	...
	_render: function() {
		...
		(function($){
			...
			if(!_amcn_dfp.last_html || _amcn_dfp.last_html != $('body').html()) {
				...
				var hasCompanion = $('.brightcove-video-template').data('adtype') === 'companion';
				googletag.cmd.push(function() {
					$(ads).each(function(i, ad){
						try {
							...
							var slot = null;
							if(pos === 'mid' && hasCompanion) {
								slot = googletag.defineSlot(cfg.ad_unit_path, def.sizes, $(this).attr('id'))
									.addService(googletag.companionAds())
									.addService(googletag.pubads())
									.setTargeting("pos", pos_name);
								if(window.console)
									console.log('companion mid: ', pos_name, cfg.ad_unit_path, def);
							}
							else if(hasCompanion) {
								var video_id = $('div.video-box.brightcove-video-template').data('video_id');
								slot = googletag.defineSlot(cfg.ad_unit_path, def.sizes, $(this).attr('id'))
									.addService(googletag.pubads())
									.setTargeting("pos", pos_name);
								if(window.console)
					...
					// for videoads
					var qp = _amcn_dfp.query_param();
					if(hasCompanion && typeof qp.QA === 'string' && qp.QA === 'true') {
						var video_id = $('div.video-box.brightcove-video-template').data('video_id');
				...
				googletag.cmd.push(function() {
					$.each(ads, function() {
						...
						if(window.console)
							console.log('dfp invoke:', $(this), $(this).attr('id'));
					});
				});
