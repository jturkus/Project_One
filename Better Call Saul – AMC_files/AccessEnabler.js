...
configureApassEndpoints();
$$$(function(){window.accessEnablerHelperURL+="#"+encodeURIComponent(document.location.href);
window.accessEnablerHelper=$$$("<iframe />",{name:"adobeIFrame",id:"adobeIFrame",style:"display:none",src:accessEnablerHelperURL});
accessEnablerHelper.appendTo("body")
});
var API_METHODS=["setRequestor","getAuthorization","checkAuthorization","checkAuthentication","logout","setSelectedProvider","getAuthentication","getSelectedProvider","getProxyMVPD","getMetadata","ditchTokens","setSocialNetwork","setSocialProviderDialogURL","checkPreauthorizedResources","getStorageData","completeBackgroundLogin",,"completeBackgroundLogout",,"bind","unbind"];
var ae={};
$$$.each(API_METHODS,function(index,value){ae[value]=function(){callMethod(value,fixargs(arguments))
}
});
var accessEnabler=ae;
function callMethod(){if(arguments.length>0){var argsToPost=fixargs(arguments);
if(argsToPost[0]=="setRequestor"&&argsToPost[1]){var clientInfoIndex=typeof argsToPost[1][1]!="undefined"&&$$$.isArray(argsToPost[1][1])?2:1;
if(typeof argsToPost[1][clientInfoIndex]=="undefined"||argsToPost[1][clientInfoIndex]==null){argsToPost[1][clientInfoIndex]={}
}argsToPost[1][clientInfoIndex].callSetConfig=typeof(setConfig)=="function"
}LOG.info("Calling "+argsToPost[0]);
$$$.postMessage(argsToPost,accessEnablerHelperURL,accessEnablerHelper[0].contentWindow)
}else{LOG.error()("no method to call!")
}}function getRedirectURL(url){return(url&&url!="null"&&url.length>4)?url:window.location.href
}$$$.receiveMessage(function(message){LOG.info(message);
var specialMessageHandlers={LOGIN_COMPLETE:ae.completeBackgroundLogin,LOGOUT_COMPLETE:ae.completeBackgroundLogout};
var handler=specialMessageHandlers[message.data];
if(handler){handler();
return
}var messageData={};
var parsingErrorMessage="";
try{messageData=JSON.parse(message.data);
if(messageData.type=="adobepass"){messageData=messageData.payload
}else{parsingErrorMessage="Message type is not adobepass."
}}catch(error){parsingErrorMessage="Message is not JSON."
}if(parsingErrorMessage.length>0){LOG.warn(parsingErrorMessage);
return
...
if(methodName==="preauthorizedResources"){if(messageParams.length==0){messageParams.push(new Array())
}}var url;
switch(methodName){case"reload":location.reload();
break;
case"openIFrame":var widthInt=parseInt(messageParams[1]);
var heightInt=parseInt(messageParams[2]);
createIFrame(widthInt,heightInt);
url=messageParams[0]+"&"+$$$.param({redirect_url:getRedirectURL(messageParams[3])});
LOG.info(url);
if($$$("#mvpdframe").length>0){$$$("#mvpdframe").attr("src",url)
}else{window.open(url,"mvpdframe")
}break;
case"navigateToURL":url=messageParams[0]+"&"+$$$.param({redirect_url:getRedirectURL(messageParams[1])});
LOG.info(url);
if(messageParams[2]){window.open(url,"mvpdwindow")
}else{window.location.href=url
}break;
case"doBackgroundLogout":createHiddenIframe("com_adobe_pass_background_logout",messageParams[0]);
break;
default:propagateCallback(methodName,messageParams);
break
}});
function propagateCallback(methodName,messageParams){LOG.info("Callback: "+methodName);
var fn=window.eval.call(window,methodName);
if(typeof(fn)=="undefined"){var debugMessage="Initial message: "+message.data+"\n\n";
debugMessage+="Calling function: "+methodName+"\n";
debugMessage+="with parameters: "+messageParams;
